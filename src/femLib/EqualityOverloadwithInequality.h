...
class Ex2ConstraintIneque : public FunctionConstraints
{
  ...
  public:
	...
	virtual const void setEquality(){
		equality = true;
	}

	virtual const void setINEquality(){
		equality = false;
	}
	...
	virtual const VectorXd &getEqualityConstraintsTargetValues()
	{
		...
		if(equality == true){
			sizeB = 2 * nbMeshPoints + 1;
		} else {
			sizeB = 2 * nbMeshPoints; //Not the last row
		}
	...
	virtual const Eigen::SparseMatrix<double> getA(const VectorXd &p)
	{
		...
		if(equality == true){
			A.resize(nbMeshPoints * 2 + 1, nbMeshPoints * 2); //One line more for the mass constraint
		} else {
			A.resize(nbMeshPoints * 2, nbMeshPoints * 2); //One line more for the mass constraint
		}
	...
	virtual const double getxCOM(const VectorXd& p){
		double totalMass = getTotalMass();
		double sum = 0;
		int nbMeshPoints = this->simParent->x.rows()/2;
		...
		for (int i = 0; i < nbMeshPoints; i++)
		{
			sum += this->simParent->m[2*i]*p[2*i];
		}
	...
	virtual const VectorXd& getInequalityConstraintValues(const VectorXd& p) {
		...
		ineqConstraintVals[0] = getxCOM(p);
		...
		ineqConstraintVals = (A * p).row(p.rows()-1); //Compute A(p)
		*/
		//std::cout << "ineqConstraintVals" << ineqConstraintVals << std::endl;
	...
	virtual const void printMass(){
		std::cout << "AVG : " << getAvgCoordsSupport() << std::endl;
		std::cout << "Max : " << getMaxSupport() << std::endl;
		std::cout << "Min : " << getMinSupport() << std::endl;
		std::cout << "xCOM : " << getxCOM(this->simParent->x) << std::endl;
